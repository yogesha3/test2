/**
 * Received Referrel Details activity
...
public class ReferrelDetailsActivity extends FragmentActivity implements View.OnClickListener {

    private final String TAG="ReferrelDetailsActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_referrel_details);
    ...
    public void initView(){
        ...
        try{
            if (AppUtils.isOnline(ReferrelDetailsActivity.this)) {
                mProgressHUD = ProgressHUD.show(ReferrelDetailsActivity.this,"", true,true);
                mProgressHUD.setCancelable(false);
                callWebservice();
            }
            else{
                ErrorMsgDialog.showErrorAlert(ReferrelDetailsActivity.this, "", getString(R.string.wrng_str_lost_internet_error));
            }
        }catch (Exception ex){
            ex.printStackTrace();
        }
            ...
            public void onClick(View v) {
                try {
                    if(mEditTextMessageComment.getText().toString().length()>0) {
                        ...
                        ErrorMsgDialog.showErrorAlert(ReferrelDetailsActivity.this, "", getString(R.string.not_comment_blanck));
                    }
                }catch (Exception e){
                    e.printStackTrace();
                }
    ...
    private void callWebservice() throws Exception {
        ...
        RestAdapter restAdapter = new RestAdapter.Builder().setEndpoint(WebServiceConstants.sWEBSERVICE_URL).setRequestInterceptor(WebServiceUtils.getRequestInterceptor(WebServiceConstants.RECEIVED_REFERREL_DETAILSL_ACTION_NAME, WebServiceConstants.REFERRALS_CONTROL_NAME, ReferrelDetailsActivity.this)).build();
    ...
    /**
     * THis method is use to call for getting all comment for selected received referral id,
     * webservice and get successful  response or  error from server
     */
    private void callCommentWebservice() throws Exception {
        //setting timeout for reading data as well as for connecting with client

        RestAdapter restAdapter = new RestAdapter.Builder().setEndpoint(WebServiceConstants.sWEBSERVICE_URL).setRequestInterceptor(WebServiceUtils.getRequestInterceptor(WebServiceConstants.RECEIVED_REFERREL_COMMENT_ACTION_NAME, WebServiceConstants.REFERRALS_CONTROL_NAME, ReferrelDetailsActivity.this)).build();
        RequestApi requestApi = restAdapter.create(RequestApi.class);
    ...
    private void callAddCommentWebservice() throws Exception {
        ...
        RestAdapter restAdapter = new RestAdapter.Builder().setEndpoint(WebServiceConstants.sWEBSERVICE_URL).setRequestInterceptor(WebServiceUtils.getRequestInterceptor(WebServiceConstants.RECEIVED_REFERREL_ADD_COMMENT_ACTION_NAME, WebServiceConstants.REFERRALS_CONTROL_NAME, ReferrelDetailsActivity.this)).build();
        RequestApi requestApi = restAdapter.create(RequestApi.class);
                    ...
                    if (responseModel != null) {
                        ...
                        if (comment_Response != null) {
                            ...
                            if (comment_Response.getCode().equals(ApplicationConstants.SUCCESS_CODE)) {
                                ErrorMsgDialog.showErrorAlert(ReferrelDetailsActivity.this, "", comment_Response.getMessage().equals("") ? getString(R.string.wrng_str_no_record) : comment_Response.getMessage(), new OkPressed());
                                mEditTextMessageComment.setText("");
                                //callCommentWebservice();
                            } else {
        ...
        public void alertAction(boolean select) {
            try {
                if (AppUtils.isOnline(ReferrelDetailsActivity.this)) {
                    ...
                    intentService = new Intent(ReferrelDetailsActivity.this, CommentService.class);
   ...
   /**
    ...
    public void setCommentONUI(ReferrelCommentResponseModel comment_Response){
        ...
        if(null!=commentlistAdapter && comment_Response.getResult().size()>commentlistAdapter.getCount()){
            commentlistAdapter=new CommentListAdapter(ReferrelDetailsActivity.this,comment_Response.getResult(),null);
    ...
    public void setValuesONUI(final ReferrelDetailsResponseModel get_Response){
        ...
        if(get_Response.getResult().getFirst_name()!= null){

            toNameStr =get_Response.getResult().getFirst_name();
            String s1 = toNameStr.substring(0, 1).toUpperCase();
            toNameStr = s1 + toNameStr.substring(1);

        }

        if(get_Response.getResult().getLast_name() != null ){
            String s1 = get_Response.getResult().getLast_name().substring(0, 1).toUpperCase();
            toNameStr =  toNameStr+" "+ s1 + get_Response.getResult().getLast_name().substring(1);
        }
...
//City, ZIP Code, State, Country
    ...
    private ReceivedReferralCommentModel getAddCommentRequestModel() throws Exception{
        ...
        mReceivedReferralCommentModel.setDetailPage(WebServiceConstants.RECEVIED_LIST_PAGE);
    ...
    private ReferralsDetailsRequestModel getRequestModel() throws Exception{
        ...
        mReferralsDetailsRequestModel.setDetailPage(WebServiceConstants.RECEVIED_LIST_PAGE);
    ...
    public void onClick(View view) {
        if(view == mBackButton){
            ...
            if(null!=getResponse && getResponse.getCode().equals("200")) {
                Intent editReferral = new Intent(ReferrelDetailsActivity.this, ReceivedRererralEditActivity.class);
